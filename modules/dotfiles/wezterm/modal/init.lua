...
local function enable_defaults(url)
	-- Add modal/modes directory to package path
	local modal_dir = wezterm.config_dir .. "/modal/modes/"
	package.path = package.path .. ";" .. modal_dir .. "?.lua"
end
...
---sets the current modal status to the left status
---@param window any
---@param name? string
local function set_left_status(window, name)
	if name then
		local mode = modes[name]
		window:set_left_status(mode.status_text)
	else
		local mode = get_mode(window)
		if mode then
			window:set_left_status(mode.status_text)
...
return {
	...
	set_left_status = set_left_status,
