...
- [ ] T05 Retirer les entrées HM des lots terminal puis sensible
  ...
  Status: En cours — entrées HM retirées pour `nvim`, `wezterm`, `tmux`, `yazi`, `sesh`, `television`, et `~/.config/zsh/**`. Les fichiers racine `~/.zshenv`/`~/.zprofile` sont conservés côté HM (bootstrap shell). Reste les autres composants sensibles (`doom/emacs`, `hammerspoon`).
  ...
  - T05-C `hm-switch` + smoke test zsh (nouveau terminal) avant de toucher `doom/emacs`.
  - T05-D Retirer ensuite les blocs `doom`/`emacs`, valider démarrage Emacs + Doom sync.
  - T05-E Retirer enfin `hammerspoon`, valider reload runtime.
